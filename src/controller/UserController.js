...
exports.UserRegister = async (req, res) => {
  try {
    ...
    if (existing.length > 0) {
      return res.json({
        status: "warning",
        response: `Another user already exists with ${email}`,
      });
    }
...
exports.UserLogin = async (req, res) => {
  try {
    ...
    if (existing.length == 0) {
      return res.json({
        status: "warning",
        response: `User does not exist with ${email}`,
      });
    }
    ...
    res.json({
      ...
      userInfo: { token: token, email: email },
    });
  } catch (error) {
    res.json({ status: "error", response: error.message });
  }
...
exports.OTPRequest = async (req, res) => {
  try {
    ...
    const otpGenerate =
      Math.round(Math.random() * (999999 - 100000 + 1)) + 100000;
    const updateOTP = await UserModel.update(
      { email: email },
      { opt: otpGenerate }
    );
    if (!updateOTP) {
      return res.json({
        status: "warning",
        response: `OTP generation failed. Try again later`,
      });
    }
    await SendEmail(email, "OTP request for password reset", otpGenerate);
    res.json({
      status: "success",
      response: `OTP request successful. Check your email at ${email}`,
    });
  } catch (error) {
    res.json({ status: "error", response: error.message });
  }
...
exports.OTPVerified = async (req, res) => {
  try {
    ...
    res.json({
      status: "success",
      response: "Your OTP has been verified. Please reset your password.",
    });
  } catch (error) {
    res.json({ status: "error", response: error.message });
  }
...
exports.ResetPassword = async (req, res) => {
  try {
    ...
    if (!updatePassword) {
      return res.json({
        status: "warning",
        response: `Password reset failed. Try again later`,
      });
    }
    await SendEmail(
      email,
...
exports.UpdateProfile = async (req, res) => {
  try {
    ...
    if (existing.length > 0) {
      return res.json({
        status: "warning",
        response: `Another user already using this ${email}`,
      });
    }
    const salt = await bcrypt.genSalt(10);
    const hashedPassword = await bcrypt.hash(password, salt);
...
exports.DeleteUser = async (req, res) => {
  try {
    ...
    if (!deleteUser) {
      return res.json({
        status: "warning",
        response: "User delete failed. Try again later",
      });
    }
    return res.json({
      status: "success",
